...
RS256::RS256()
{
  ...
  /* compute feedback             */
  Sigui8  feedback_byte;
  feedback_byte.SetSize(4096);

  LOOP(ii,256){ /* datain = ii (pol representation) */
    LOOP(jj,16){ /* convention: fbk 0 <--> pol 15 */
      feedback_byte[ii*16+jj] = gf.pow2pol[gf256_mulpow(genpoly[15-jj],gf.pol2pow[ii])]; 
    }
  }
  feedback = Sigv16qi(feedback_byte);
  /* alloc reg */
  reg.SetSize(16); /* convention: reg 0<-->pol 15*/
}

__v16qi RS256::encode239(const Sigui8 &I_239_InfoBytes)
{
  ...
  LOOP(ii,239){
    ...
    dataFeedBack   = dataIn ^ reg[0]; /* because genpoly[16] = 1 */
    mm_feedback    = feedback[dataFeedBack]; 

    /* right shift 1byte */
    *mm_reg = _mm_srli_si128( *mm_reg, 1);
    /* adding feedback */
...
void RS256::encode1pck(const Sigui8 &IO_204Bytes)
{
  ...
  memcpy(IO_204Bytes.bufferA+188,(char*)mm_reg,16);
}
