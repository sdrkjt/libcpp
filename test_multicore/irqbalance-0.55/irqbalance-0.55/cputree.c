...
static void do_one_cpu(char *path)
{
	struct cpu_core *cpu;
	FILE *file;
	char new_path[PATH_MAX];char buffer[4396];
	...
	printf(" banned_cpus--> %s \n",buffer);
	
	if (cpus_intersects(cpu->mask, banned_cpus)) {
		free(cpu);
		/* even though we don't use the cpu we do need to count it */
		cpumask_scnprintf(buffer, 4096, cpu->mask);
		printf(" cpusMask--> %s ",buffer);
		printf("we don't use the cpu we do need to count it\n");
